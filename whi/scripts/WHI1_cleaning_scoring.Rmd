...
attr(surveys_scored$demo_race, "label") <- "cleaned racial category. 'multiracial' if multiple categories were selected. (Latinx/Hispanic was not included)"
...
attr(surveys_scored$arkansas_HC_location_AB_AK, "label") <- "In the last 12 months, have you or people in your household gotten health services at any of the places below? Choose all that apply"
attr(surveys_scored$healthy_needs_AB_3, "label") <- "Which of the things below would help you or others in your household to get healthy or stay healthy? Please select all that apply"
attr(surveys_scored$adult_HC_barriers_AB_4, "label") <- "In the last 12 months, which of these problems kept you or other adults in your household from getting the health care or medicine you needed? (Choose all that apply)"
attr(surveys_scored$adult_mentalHC_barriers_AB_5, "label") <- "In the last 12 months, which of these problems kept you or other adults in your household from getting the mental health care you needed? (Choose all that apply)"
attr(surveys_scored$child_mentalHC_barriers_AB_6, "label") <- "In the last 12 months, which of these problems kept children in your household from getting the mental health care they needed? (Choose all that apply)"
attr(surveys_scored$teleheath_barriers_AB_7, "label") <- "In the last 12 months, which of these problems kept you or anyone in your household from getting a telehealth or video visit with a health care worker? Choose all that apply"
```
...
AB_2_response <- surveys_scored %>%
  ...
  select(userid:AB_2_important_3) # because the survey asked to select 3 important options so all other transformed rankings were removed from the dataset
...
AB_33 <- surveys_scored %>%
  ...
  select(-option) %>%
  spread(col_name, select) %>% # transform the long-format back to a wide-format
  select(userid:AB_33_rank_3) # because the survey asked to select the top 3 so all other transformed rankings were removed from the dataset
...
surveys_scored <- surveys_scored %>%
    dplyr::mutate(across(c(SDOH_1, SDOH_2, SDOH_4:SDOH_6), # recode binary variables into a Boolean
                ~ case_when(
                  ...
                  . == "Yes" ~ TRUE,
                  . == "Unsure" ~ NA_real_,
                  . == "I prefer not to answer" ~ NA_real_
                ), 
                .names = "{.col}_R" #add 
              )) %>%
...
surveys_scored <- surveys_scored %>%
  dplyr::mutate(CH_1_R = case_when(CH_1 == "Yes" ~ TRUE, 
                            ...
                            CH_1 == "No" ~ FALSE, 
                            CH_1 == "Don’t know / Not sure" ~ NA_real_)) %>%
  dplyr::mutate(across(CH_2:CH_3_15_13, 
                ~ case_when(
                  . == "Yes" ~ TRUE,
                  . == "No" ~ FALSE, 
                  . == "Don’t know / Not sure" ~ NA_real_
                ), 
                .names = "{.col}_R")) %>%
