...
f_varname <- c(value_between(chr_col_names, "Demo_gender", "VIS_Strengths"),
               ...
               value_between(chr_col_names, "ValueProp_9", "ValueProp_10_10"), 
               value_between(chr_col_names, "Sex_prolific", "P4.3"),
               value_between(chr_col_names, "status_prolific_api", "SWLS_tot_f"),"Fluent.languages_prolific")
...
prompt_df_rn <- rename_var_std(prompt_df_rc_bool)

# create a summary table
prompt_rn_sum <- org_varname(prompt_df_rn) %>%
  mutate(ori_name = names(prompt_df_rc_bool))
...
# create a dictionary for all categorical variables

```{r}
# extract categorical variables(factors and booleans)
varname_cat_df <- prompt_rn_sum_new %>%
  filter(var_class %in% c("factor", "logical"))

varname_cat <- varname_cat_df
```
...
#write.csv(prompt_rn_sum_new, here("outputs", paste("prolific_prompt_var_summary_", current_date, ".csv", sep = "")), row.names = F)
