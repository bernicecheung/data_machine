...
ltab_df_std <- read.csv(here("outputs", "ltaw_df_std_110823.csv"))

# load variable naming list
survey_varname_std <- read.csv(here("outputs", "ltaw_var_summary_110823.csv"))
...
scale_df <- survey_std %>%
  filter(item_suffix == "") %>%
  select(-item_suffix, -calculation)
