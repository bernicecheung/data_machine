...
survey <- import(here("inputs", "Let's Talk About Well-Being_August 2, 2023_07.59.sav"))
survey$survey_name <- "ltaw"
...
survey_tidy <- survey_tidy %>% left_join(prolific, by = "userid")
...
# survey_tidy <- survey_tidy %>% left_join(dupes, by = "userid")
...
attr(survey_tidy$flag_failed_recording_test, "label") <- "TRUE if participants failed teh recording test. NAs were retained"
...
# add label
attr(survey_tidy$flag_failed_attention, "label") <- "TRUE if at least 1 attention checks failed; FALSE if 1 attention check passed + 1 missing or failed both; NA if both attention checks are missing"
attr(survey_tidy$flag_failed_attention_n, "label") <- "total number of attention checks failed (NA not counted as failure)"
```
...
attr(survey_tidy$flag_no_survey, "label") <- "TRUE if participants didn't provide any survey data; FALSE if at least 1 survey item was recorded"
```
...
attr(survey_tidy$max_identical, "label") <- "The number if identical survey responses in a seqeunce"
attr(survey_tidy$max_identical_perc, "label") <- "The perceptage of identical survey responses in a seqeunce out of all survey items"
```
...
attr(survey_tidy$duration, "label") <- "The time participants spent on the survey in minutes"
```
...
# add labels
attr(survey_tidy$flag_dupe_survey, "label") <- "TRUE if this observation came from a user who recorded more than one observation; FALSE if this observation came from a unique user"
attr(survey_tidy$flag_most_complete, "label") <- "TRUE if this observation came from a unique user, or was the most complete one among duplicated observations; SAME if the duplicated observations had same progress;
...
## recode numbers into variable names
```{r}
...
# surveys_scored <- surveys_scored %>%
#   mutate(across(Demo_ethnicity_1:Demo_ethnicity__99, ~ case_match(., 1 ~ TRUE))) %>%
